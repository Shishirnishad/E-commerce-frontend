...
function CustomerProfileEdit({ customer, onUpdate, onBack }) {
  ...
  const [preview, setPreview] = useState(customer.CPicName || "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append("CEmail", form.CEmail);
      fd.append("CContact", form.CContact);
      fd.append("CAddress", form.CAddress);
      if (file) fd.append("file", file);

      const res = await axios.put(
        `${REACT_APP_BASE_API_URL}/customer/update/${customer.CUserId}`,
        fd
      );
      ...
      if (onUpdate) {
        onUpdate({
          ...
          CPicName: preview || customer.CPicName,
        });
      }
